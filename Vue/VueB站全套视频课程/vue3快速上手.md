...
![image-20230226142652854](vue3快速上手.assets/image-20230226142652854.png)
...
- 两个小“坑”：
  ...
  - 监视reactive定义的响应式数据时：oldValue无法正确获取、强制开启了深度监视（deep配置失效，无论嵌套的多深，都可以监视到）。
  - 监视reactive定义的响应式数据中某个属性时：deep配置有效。
  
  注意：通过watch函数监听ref所引用的数据时，不需要通过".Value"来监听，如果是监测的是“对象”时，则需要添加deep深度检测。
  ...
  */
  ...
  //此处的意思是：job的为一个对象，所以需要添加deep才有效，否者无法监视里面的值变化。
  ```
...
- Vue3.0也提供了 Composition API 形式的生命周期钩子，与Vue2.x中钩子对应关 系如下：
  - `beforeCreate`===>`setup()`
  - `created`=======>`setup()`
...
下面是封装好的hook函数代码，在要使用的组件中，直接引用import就行了。

![image-20230226192916507](vue3快速上手.assets/image-20230226192916507.png)
